...
function ChangelogBackground() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <motion.div
        className="absolute -top-60 right-0 h-[700px] w-[700px] rounded-full bg-cyan-500/[0.05] blur-[140px]"
        animate={{ scale: [1, 1.1, 1], opacity: [0.4, 0.7, 0.4] }}
        transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-0 -left-40 h-[500px] w-[500px] rounded-full bg-violet-500/[0.04] blur-[120px]"
...
function ChangelogHero() {
  ...
  return (
    <motion.section
      ...
      className="relative overflow-hidden border-b border-white/5 pb-20 pt-28 sm:pt-36"
    >
      <ChangelogBackground />
      ...
      >
        <motion.div
          ...
          className="mb-7 inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-500/10 px-5 py-2 text-xs font-medium text-cyan-400 backdrop-blur-sm"
        >
        ...
        >
          New features, improvements, and fixes shipped by the FormaOS team.
        ...
        <motion.div
          ...
          className="mt-8 inline-flex flex-wrap items-center justify-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-6 py-4 backdrop-blur-sm"
        >
  ...
  return (
    ...
    >
      ...
      {TAG_FILTERS.map((filter) => {
        ...
        const FilterIcon = filter === 'All' ? Tag : (TAG_ICONS[filter] ?? Tag);
        return (
          <button
            key={filter}
            ...
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-all duration-300 ${
              isActive
                ? 'border-cyan-400/30 bg-cyan-400/10 text-cyan-400 shadow-sm shadow-cyan-400/5'
                : 'border-white/[0.06] bg-white/[0.01] text-slate-500 hover:bg-white/[0.04] hover:text-slate-300'
            }`}
          >
...
function YearDivider({ year }: { year: number }) {
  return (
    ...
    >
      <div className="absolute -left-[29px] h-6 w-6 rounded-lg bg-gradient-to-br from-white/[0.08] to-white/[0.02] ring-1 ring-white/10 flex items-center justify-center">
        <span className="text-[9px] font-bold text-cyan-400">★</span>
      </div>
      <span className="text-sm font-bold uppercase tracking-widest text-slate-400">
        {year}
      </span>
      <div className="h-px flex-1 bg-gradient-to-r from-white/10 via-cyan-500/10 to-transparent" />
    </motion.div>
  );
}
...
function EntryCard({
  ...
  const TagIcon = TAG_ICONS[entry.tag] ?? Tag;
  ...
  return (
    ...
    >
      {/* Timeline dot */}
      {isMajor ? (
        <div className="absolute -left-[35px] top-2 h-4 w-4 rounded-full bg-violet-500/20 ring-2 ring-violet-400/80 shadow-[0_0_14px_rgba(139,92,246,0.4)]">
          <motion.div
        ...
        <div className="absolute -left-[33px] top-2 h-3 w-3 rounded-full bg-canvas-800 ring-2 ring-cyan-500/60" />
      )}
      ...
      {/* Card */}
      <div
        className={`group rounded-2xl border p-6 transition-all duration-500 backdrop-blur-sm ${
          isMajor
            ? 'border-violet-500/20 bg-violet-500/[0.03] shadow-[0_0_40px_-12px_rgba(139,92,246,0.15)] hover:shadow-[0_0_60px_-12px_rgba(139,92,246,0.2)]'
      ...
      >
        ...
        >
          <div className="flex flex-wrap items-center gap-3">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${entry.tagColor}`}
            >
              <TagIcon className="h-3 w-3" />
              {entry.tag}
            </span>
            <h2 className="text-base font-bold text-white group-hover:text-white/95">
              {entry.title}
            </h2>
          </div>
          <motion.div
        ...
        {!expanded && (
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-500">
            <span>{entry.items.length} changes</span>
        ...
        <AnimatePresence initial={false}>
          {expanded && (
            ...
            >
              <ul className="mt-5 space-y-3 border-t border-white/[0.04] pt-5">
                  ...
                  >
                    <span
                      className={`mt-2 h-1.5 w-1.5 shrink-0 rounded-full transition-colors ${
                        isMajor
                          ? 'bg-violet-400/50 group-hover/item:bg-violet-400'
                          : 'bg-cyan-500/50 group-hover/item:bg-cyan-400'
                      }`}
                    />
                    <span className="group-hover/item:text-slate-300 transition-colors">
                      {item}
                    </span>
                  </motion.li>
                ))}
...
function TimelineSpine() {
  ...
  return (
    <div ref={ref} className="absolute left-0 top-0 h-full w-px">
      {/* Static ghost line */}
      <div className="absolute inset-0 bg-gradient-to-b from-cyan-500/20 via-white/5 to-transparent" />
      {/* Animated progress line */}
      <motion.div
        className="absolute left-0 top-0 w-full origin-top bg-gradient-to-b from-cyan-400/60 via-blue-400/40 to-violet-400/30"
...
function StatsSummary() {
  const totalChanges = ENTRIES.reduce((acc, e) => acc + e.items.length, 0);
  const stats = [
    {
      label: 'Releases',
    ...
    },
    {
      ...
      value: ENTRIES.filter((e) => e.tag === 'Major Release').length.toString(),
      color: 'text-violet-400',
    },
    {
      label: 'Security Updates',
      value: ENTRIES.filter((e) => e.tag === 'Security').length.toString(),
  ...
  return (
    <motion.div
      ...
      className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-12"
    >
      {stats.map((stat, i) => (
        <motion.div
          key={stat.label}
          ...
          className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4 text-center backdrop-blur-sm"
        >
...
function SubscribeCTA() {
  return (
    <motion.div
      ...
      transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
      className="relative mt-20 overflow-hidden rounded-3xl border border-white/[0.06]"
    >
      ...
      <div className="relative p-10 sm:p-14 text-center">
        <motion.div
          ...
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          className="inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-1.5 text-xs font-semibold text-cyan-400 mb-5"
        >
        ...
        <p className="mt-3 text-sm text-slate-400 max-w-md mx-auto">
          Subscribe to release notes or follow us for product updates. Every
          release is documented with full transparency.
        </p>
        <motion.a
          href="/contact"
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.98 }}
          className="group mt-7 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-7 py-3 text-sm font-semibold text-white shadow-lg shadow-cyan-500/20 transition-shadow hover:shadow-xl hover:shadow-cyan-500/30"
        >
          Get release notes
          ...
          <div className="space-y-14 pl-8">
            <AnimatePresence mode="popLayout">
              {filteredEntries.map((entry, index) => {
                ...
                lastYear = entryYear;
                const isLatest = index === 0 && activeFilter === 'All';
