...
function renderCell(value: Cell) {
  ...
  if (value === '—')
    return <span aria-label="Not included" className="opacity-40">—</span>;
  return <span className="font-medium">{value}</span>;
}

/**
 * MxCapabilityMatrix — dense maximalist comparison table on cream block.
 * No card frame, no sticky sidebar; bold rule above group rows, mustard
 * tint on the featured column. Mobile collapses to per-group stacked
 * dl. Same data shape as the editorial version, restyled.
 */
export function MxCapabilityMatrix() {
  return (
    <MxBlock tone="cream" id="capability-matrix" rail="§02 / MATRIX">
      <MxContainer width="full">
        <div className="grid grid-cols-12 items-end gap-6 border-b-[6px] border-[var(--mx-ink)] pb-6 mb-10">
          <div className="col-span-12 lg:col-span-7">
            <p className="mx-eyebrow opacity-70">§02 / CAPABILITY MATRIX</p>
          ...
          <div className="col-span-12 lg:col-span-5">
            <MxLead className="text-[var(--mx-ink-mid)]">
              ...
              audit and it is not listed below, <em>ask us</em>.
            </MxLead>
          </div>
        </div>
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="mx-table min-w-[56rem]">
            <thead>
              <tr>
                ...
                {COLUMNS.map((c) => (
                  <th
                    key={c.key}
                    scope="col"
                    className={`mx-table__center ${c.featured ? 'mx-table__featured' : ''}`}
                  >
                    <div className="flex flex-col items-center gap-1">
                      <span className="font-serif text-[1rem] font-medium normal-case tracking-normal">
                        {c.label}
                        {c.featured ? (
                          <span className="ml-1 text-[var(--mx-oxblood)]">
                            ★
                          </span>
                        ) : null}
                      </span>
                      <span className="mx-caption opacity-70">{c.price}</span>
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {GROUPS.map((group) => (
                <Fragment key={group.title}>
                  <tr className="mx-table__group">
                    <td colSpan={COLUMNS.length + 1}>{group.title}</td>
                  </tr>
                  {group.rows.map((row) => (
                    <tr key={row.label}>
                      <td>
                        <div className="font-serif text-[1rem]">
                          {row.label}
                        </div>
                        {row.hint ? (
                          <div className="mx-caption mt-1 opacity-70">
                            {row.hint}
                          </div>
                        ) : null}
                      </td>
        ...
        <div className="space-y-8 md:hidden">
          {GROUPS.map((group) => (
            <div key={group.title}>
              <h3 className="mx-eyebrow border-b-[3px] border-[var(--mx-ink)] pb-2">
                {group.title}
              </h3>
              <dl className="mt-4 space-y-4">
                {group.rows.map((row) => (
                  <div
                    key={row.label}
                    className="border-b border-[var(--mx-paper-rule)] pb-3"
                  >
                    <dt className="font-serif text-[1rem]">{row.label}</dt>
                    {row.hint ? (
                      <p className="mx-caption mt-1 opacity-70">{row.hint}</p>
                    ) : null}
                    <dl className="mt-3 grid grid-cols-2 gap-2 text-[0.875rem]">
                      {COLUMNS.map((c) => (
                        <div
                          key={c.key}
                          className="flex items-baseline justify-between"
                        >
                          <dt className="mx-eyebrow opacity-70">
                            {c.label}
                            {c.featured ? (
                              <span className="ml-1 text-[var(--mx-oxblood)]">
                                ★
        ...
        <MxRule className="mt-10" />
        <p className="mx-caption mt-3 opacity-70">
          <span className="text-[var(--mx-oxblood)]">★</span> Most popular
          &mdash; selected by the majority of registered NDIS providers with
          10&ndash;25 staff. ● included; &mdash; not included.
        </p>
      </MxContainer>
    </MxBlock>
  );
