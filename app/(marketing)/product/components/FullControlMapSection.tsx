...
const frameworkChips = [
  { id: 'iso', color: 'bg-teal-400', glow: 'shadow-teal-400/40', label: 'ISO 27001', count: 21 },
  { id: 'soc', color: 'bg-indigo-400', glow: 'shadow-indigo-400/40', label: 'SOC 2', count: 21 },
  { id: 'nist', color: 'bg-sky-400', glow: 'shadow-sky-400/40', label: 'NIST CSF', count: 21 },
  { id: 'hipaa', color: 'bg-pink-400', glow: 'shadow-pink-400/40', label: 'HIPAA', count: 20 },
  { id: 'gdpr', color: 'bg-violet-400', glow: 'shadow-violet-400/40', label: 'GDPR', count: 19 },
  { id: 'pci', color: 'bg-orange-400', glow: 'shadow-orange-400/40', label: 'PCI DSS', count: 19 },
  { id: 'cis', color: 'bg-emerald-400', glow: 'shadow-emerald-400/40', label: 'CIS Controls', count: 22 },
];
  ...
  return (
    <section className="mk-section relative overflow-hidden py-24 lg:py-32">
      ...
      <div className="relative z-10 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-12">
        {/* Header */}
        <ScrollReveal variant="blurIn" range={[0, 0.3]} className="mb-12 text-center lg:mb-16">
          ...
          <h2 className="mb-5 text-3xl font-bold sm:text-4xl lg:text-5xl xl:text-6xl tracking-tight">
            Every Control.{' '}
          ...
          <p className="mx-auto mb-10 max-w-3xl text-base leading-relaxed text-gray-400 sm:text-lg lg:text-xl">
            147 controls across 7 enterprise frameworks — all interconnected, all
            continuously monitored, all audit-ready. Hover any node to trace its
            ...
            {frameworkChips.map((fw) => (
              ...
              >
                <div className="relative">
                  <div className={`h-2.5 w-2.5 rounded-full ${fw.color} ${fw.glow} shadow-[0_0_6px]`} />
                  <div className={`absolute inset-0 h-2.5 w-2.5 rounded-full ${fw.color} animate-ping opacity-20`} />
                </div>
                <span className="text-xs font-medium text-white/80 sm:text-sm">{fw.label}</span>
        ...
        <ScrollReveal variant="fadeUp" range={[0, 0.35]}>
          <div className="relative overflow-hidden rounded-3xl border border-white/[0.06] bg-gradient-to-br from-gray-950/95 via-[#050a18] to-[#030710] shadow-2xl shadow-black/50 ring-1 ring-white/[0.03]">
            ...
            <div className="pointer-events-none absolute inset-x-5 top-5 z-20 hidden lg:flex items-start justify-between gap-4">
              <div className="max-w-[35%] rounded-xl border border-teal-400/12 bg-slate-950/75 px-5 py-3.5 backdrop-blur-md">
                ...
                <p className="text-xs text-slate-300/80 leading-relaxed">
                  All 147 controls mapped across 7 frameworks with cross-linked compliance chains.
                </p>
              </div>
              <div className="max-w-[35%] rounded-xl border border-indigo-400/12 bg-slate-950/75 px-5 py-3.5 backdrop-blur-md text-right">
                ...
                <p className="text-xs text-slate-300/80 leading-relaxed">
                  Cross-framework intelligence automatically maps shared obligations.
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3.5 rounded-lg bg-slate-950/70 px-4 py-2 backdrop-blur-md border border-white/[0.05]">
                {statBlocks.map((s, i) => (
                  <span key={s.label} className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-mono ${s.accent}`}>{s.value}</span>
                  </span>
                ))}
              </div>
            ...
            <div className="relative aspect-[4/3] min-h-[400px] sm:aspect-[16/10] sm:min-h-[520px] lg:aspect-[16/9] lg:min-h-0 xl:aspect-[2/1]">
          ...
          <div className="mt-6 text-center">
            <p className="inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-slate-950/50 px-5 py-2.5 text-xs text-slate-400 backdrop-blur-sm sm:text-sm">
              ...
              Hover any node to trace cross-framework compliance chains
