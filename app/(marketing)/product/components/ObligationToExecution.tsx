...
const lifecycle = [
  { label: 'Obligation', icon: FileText, detail: 'Framework requirement mapped' },
  { label: 'Control', icon: Shield, detail: 'Ownership and cadence set' },
  { label: 'Task', icon: CheckSquare, detail: 'Routed to the owner' },
  { label: 'Evidence', icon: Lock, detail: 'Artifacts linked and sealed' },
  { label: 'Audit', icon: BarChart3, detail: 'Exportable trail' },
] as const;
    ...
    <section className="product-section product-section--process relative overflow-hidden py-24 sm:py-32">
      <div className="relative z-10 mx-auto max-w-6xl px-6 lg:px-12">
          ...
          <div className="overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.02] p-6 sm:p-10">
              ...
              <ol className="grid grid-cols-1 gap-y-8 sm:grid-cols-2 md:grid-cols-5 md:gap-4">
                {lifecycle.map((step) => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative z-10 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/[0.12] bg-[#1c1e20] text-zinc-200 transition-all duration-300 group-hover:-translate-y-1 group-hover:border-white/30 group-hover:text-white">
                        <Icon className="h-6 w-6" strokeWidth={1.6} aria-hidden="true" />
                      </div>
                      <p className="mt-4 text-sm font-semibold text-white">
                        {step.label}
                      </p>
                      <p className="mt-1 max-w-[150px] text-xs leading-relaxed text-zinc-500">
                        {step.detail}
                      </p>
                    </li>
                  );
            ...
            <div className="mt-10 border-t border-white/[0.07] pt-8">
              <div className="flex flex-wrap items-center gap-4">
                <ArrowRight
                  className="hidden h-5 w-5 text-zinc-600 sm:block"
                  aria-hidden="true"
                />
                <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl border border-white/[0.12] bg-white/[0.03]">
                  <FileCheck className="h-6 w-6 text-zinc-200" strokeWidth={1.6} aria-hidden="true" />
                </div>
                ...
                <span className="ml-auto rounded-full border border-white/[0.08] px-2.5 py-1 text-[10px] font-medium text-zinc-500">
                  Illustrative
                </span>
              </div>
            </div>
        ...
        <ScrollReveal
          variant="fadeUp"
          range={[0.1, 0.45]}
          className="mt-8 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-sm text-zinc-400"
        >
          {TRUTHS.map((t, i) => (
