...
function CommandDeck({ motionEnabled }: { motionEnabled: boolean }) {
  return (
    <motion.div
      ...
      className="mx-auto mb-8 w-full max-w-3xl rounded-2xl border border-white/[0.1] bg-white/[0.04] p-4 sm:p-5 backdrop-blur-sm"
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] pb-3">
        ...
        <span className="rounded-full border border-teal-400/20 bg-teal-500/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-teal-300">
          Enterprise
        </span>
      </div>
      ...
      <div className="mt-3 grid gap-2">
        {COMMAND_DECK_ROWS.map((row) => (
          <div
            key={row.label}
            className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.03] px-3 py-2"
          >
