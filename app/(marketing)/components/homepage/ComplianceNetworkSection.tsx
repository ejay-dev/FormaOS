...
const legendItems = [
  { color: 'bg-teal-400', glow: 'shadow-teal-400/40', label: 'Frameworks', count: '5 supported', status: 'Active' },
  { color: 'bg-indigo-400', glow: 'shadow-indigo-400/40', label: 'Controls', count: '6 mapped', status: 'Enforced' },
  { color: 'bg-emerald-400', glow: 'shadow-emerald-400/40', label: 'Evidence', count: '4 types', status: 'Verified' },
  { color: 'bg-amber-400', glow: 'shadow-amber-400/40', label: 'Tasks', count: '3 workflows', status: 'Running' },
];
  ...
  return (
    <section className="mk-section home-section home-section--network relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        {/* Centered header */}
        <ScrollReveal variant="blurIn" range={[0, 0.3]} className="mb-10 text-center lg:mb-16">
            ...
            {legendItems.map((item) => (
              ...
              >
                <div className="relative">
                  <div className={`h-3 w-3 rounded-full ${item.color} ${item.glow} shadow-[0_0_8px]`} />
                  <div className={`absolute inset-0 h-3 w-3 rounded-full ${item.color} animate-ping opacity-30`} />
                </div>
                <div className="text-left">
                  <div className="text-xs font-semibold text-white sm:text-sm sm:font-medium">{item.label}</div>
          ...
          <div className="mb-4 grid gap-2 sm:hidden">
            <div className="home-panel home-panel--soft rounded-xl border border-teal-400/15 bg-slate-950/60 px-4 py-3 text-left backdrop-blur-md">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] uppercase tracking-[0.18em] text-teal-300/80">Live Mapping</p>
              </div>
              <p className="mt-1.5 text-xs text-slate-300">Framework obligations connected to active controls.</p>
            </div>
            <div className="home-panel home-panel--soft rounded-xl border border-indigo-400/20 bg-slate-950/60 px-4 py-3 text-left backdrop-blur-md">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] uppercase tracking-[0.18em] text-indigo-300/80">Audit Readiness</p>
              </div>
              <p className="mt-1.5 text-xs text-slate-300">Task and evidence chains remain continuously verifiable.</p>
            </div>
          </div>

          <div className="home-panel home-panel--strong relative min-h-[420px] overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-gray-950/90 via-[#060c1e] to-[#040810] shadow-2xl shadow-black/40 ring-1 ring-white/[0.03] aspect-[4/3] sm:min-h-[500px] sm:aspect-[16/11] lg:min-h-0 lg:aspect-[16/7]">
            ...
            <div className="pointer-events-none absolute inset-x-4 top-4 z-20 hidden lg:flex items-start justify-between gap-3">
              <div className="home-panel home-panel--soft max-w-[48%] rounded-xl border border-teal-400/12 bg-slate-950/70 px-4 py-3 backdrop-blur-md">
                ...
                <p className="text-xs text-slate-300/80">Framework obligations connected to active controls.</p>
              </div>
              <div className="home-panel home-panel--soft max-w-[48%] rounded-xl border border-indigo-400/15 bg-slate-950/70 px-4 py-3 backdrop-blur-md text-right">
                ...
                <p className="text-xs text-slate-300/80">Task and evidence chains remain continuously verifiable.</p>
              </div>
            </div>
            ...
            <div className="pointer-events-none absolute inset-x-4 bottom-4 z-20 hidden lg:flex items-center justify-between">
              <div className="flex items-center gap-3 rounded-lg bg-slate-950/60 px-3 py-1.5 backdrop-blur-md border border-white/[0.04]">
                <span className="text-[10px] uppercase tracking-[0.14em] text-slate-500">Nodes</span>
                <span className="text-xs font-mono text-teal-300/80">18</span>
                <span className="h-3 w-px bg-slate-700" />
                <span className="text-[10px] uppercase tracking-[0.14em] text-slate-500">Edges</span>
                <span className="text-xs font-mono text-indigo-300/80">22</span>
                <span className="h-3 w-px bg-slate-700" />
                <span className="text-[10px] uppercase tracking-[0.14em] text-slate-500">Coverage</span>
                <span className="text-xs font-mono text-emerald-300/80">100%</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-slate-950/60 px-3 py-1.5 backdrop-blur-md border border-white/[0.04]">
                ...
                <span className="text-[10px] uppercase tracking-[0.14em] text-emerald-400/80">All Systems Nominal</span>
              </div>
            </div>
