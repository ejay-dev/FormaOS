...
export async function scoreEvidenceQuality(reason?: string) {
  ...
  const evidenceIds = evidenceRows.map((e: { id: string }) => e.id);
  ...
  const controlIds = Array.from(
    new Set(
      (evidenceLinks ?? [])
        .map((e: { control_id: string }) => e.control_id)
        .filter(Boolean),
    ),
  );
  ...
  const controlsById = new Map(
    (controls ?? []).map((c: Record<string, unknown>) => [c.id, c]),
  );
  const evidenceToControls = new Map<string, any[]>();
  for (const link of evidenceLinks ?? []) {
    if (!evidenceToControls.has(link.evidence_id))
      evidenceToControls.set(link.evidence_id, []);
    evidenceToControls
      .get(link.evidence_id)!
      .push(controlsById.get(link.control_id));
  }

  let scored = 0;
  for (const evidence of evidenceRows as Record<string, unknown>[]) {
    const status = String(
      evidence.verification_status || evidence.status || 'pending',
    ).toLowerCase();
    let base = 60;
    ...
    const matchBonus = relatedControls.some((c: Record<string, unknown>) => {
      ...
      return controlText
        .split(' ')
        .some((t: string) => t && evidenceText.includes(t));
    })
      ? 10
      : 0;
  ...
  await logAuditEvent({
    ...
    afterState: { scored },
    reason:
      reason && reason.trim().length ? reason.trim() : 'ai_quality_scoring',
  });
...
export async function autoMapEvidenceToControls(
  ...
  if (evidence.organization_id !== permissionCtx.orgId)
  ...
  const matchedControls = (controls ?? []).filter(
  ...
  await logAuditEvent({
    ...
    afterState: {
      matchedControls: matchedControls.map((c: { code?: string }) => c.code),
    },
