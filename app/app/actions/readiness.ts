...
export async function runAutomatedReadinessCheck(
  ...
  try {
    ...
    const result = await evaluateFrameworkControls(
      permissionCtx.orgId,
      frameworkCode,
    );
    if (!result || 'error' in result)
      throw new Error('Framework not found or no controls available');
    ...
    const existingControlIds = new Set(
      (existingMappings ?? []).map((m: any) => m.control_id),
    );
    let createdTasks = 0;
