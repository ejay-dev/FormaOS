...
type AuditPackage = {
  ...
  generatedAt: string;
  snapshot: ReturnType<typeof getOrgComplianceSnapshot> extends Promise<infer T>
    ? T
    : never;
  controls: unknown[];
  evaluations: unknown[];
...
export async function generateAuditPackageDataset(frameworkCode: string) {
  ...
  const controlIds = (controls as Array<{ id: string }>).map((c) => c.id);
  ...
  const evidenceIds = Array.from(
    new Set(
      (evidenceLinks as Array<{ evidence_id: string }>)
        .map((e) => e.evidence_id)
        .filter(Boolean),
    ),
  );
  ...
  const taskIds = Array.from(
    new Set(
      (taskLinks as Array<{ task_id: string }>)
        .map((t) => t.task_id)
        .filter(Boolean),
    ),
  );
