...
import { FileText, ShieldAlert, CheckCircle2, User } from 'lucide-react';
...
export default async function HistoryPage() {
  ...
  // 2. Fetch Logs (The History) — use canonical org_audit_logs table
  ...
  const getEventStyle = (action: string) => {
    ...
    if (action.includes('asset'))
      return {
        icon: <ShieldAlert className="h-4 w-4 text-purple-300" />,
        bg: 'bg-purple-500/10',
        border: 'border-purple-400/30',
      };
    if (action.includes('invite'))
      return {
        icon: <User className="h-4 w-4 text-amber-300" />,
        bg: 'bg-amber-400/10',
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          A permanent timeline of all governance events, policy changes, and
          incidents.
        </p>
      </div>
      ...
      <div className="relative border-l-2 border-edge-2 ml-4 space-y-8 pb-12">
        {auditLogs.map((log, _index) => {
          ...
          return (
            <div key={log.id} className="relative pl-8 group">
              {/* Timeline Dot */}
              <div
                className={`absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 border-white ring-2 ${style.bg === 'bg-sky-500/10' ? 'ring-sky-400/30 bg-sky-500' : 'ring-white/10 bg-glass-strong'}`}
              />
        ...
        <div className="relative pl-8">
          <div className="absolute -left-[5px] top-1 h-2 w-2 rounded-full bg-glass-strong" />
          <p className="text-xs text-muted-foreground italic">End of record.</p>
        </div>
      </div>
    </div>
  );
