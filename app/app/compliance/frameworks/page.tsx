  ...
  if (frameworks.length === 0) {
    return (
      <div className="rounded-2xl border border-glass-border bg-glass-subtle p-8 text-center">
        ...
        <div className="mt-4 text-sm font-semibold text-foreground">No frameworks enabled</div>
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {framework.domains.length === 0 ? (
              <span className="text-xs text-muted-foreground">No domains registered yet.</span>
            ) : (
              framework.domains.map((domain: any) => (
                <Badge key={domain.id} variant="outline" className="gap-1.5">
                  <span>{domain.name}</span>
                  <span className="text-muted-foreground">{domain.controlCount}</span>
                </Badge>
              ))
            )}
          </div>
...
async function Soc2MappingSection({ orgId }: { orgId: string }) {
  ...
  return (
    <div className="rounded-2xl border border-glass-border bg-glass-subtle p-6">
      ...
      <h3 className="mt-2 text-lg font-semibold text-foreground">Multi-framework control view</h3>
        ...
        <table className="min-w-[640px] w-full text-sm">
          <thead className="border-b border-glass-border text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium">Mapped Frameworks</th>
              <th className="px-3 py-2 text-left font-medium">Coverage</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-glass-border">
            {soc2Mappings.map((mapping) => (
              <tr key={mapping.controlId} className="transition-colors hover:bg-glass-subtle">
                <td className="px-3 py-3">
                  <div className="text-sm font-semibold text-foreground">{mapping.controlCode}</div>
                  <div className="text-xs text-muted-foreground">{mapping.controlTitle}</div>
                </td>
                <td className="px-3 py-3">
...
export default async function ComplianceFrameworksPage() {
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-500">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black text-foreground tracking-tight">Framework Library</h1>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-lg font-semibold text-foreground">SOC 2 Readiness Dashboard</div>
            <div className="text-sm text-muted-foreground">Automated evidence checks, gap analysis, and certification tracking</div>
          </div>
        </div>
