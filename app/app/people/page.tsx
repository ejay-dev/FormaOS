...
import { fetchPeopleOverview, type PeopleOverviewMember } from "@/app/app/actions/people"
...
export default function PeoplePage() {
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((member) => {
      ...
      const normalizedCompliance = (member.compliance_status ?? "active").toLowerCase()
      ...
      const matchesRole = roleFilter === ALL_FILTER || normalizedRole === roleFilter
      const matchesCompliance =
        complianceFilter === ALL_FILTER || normalizedCompliance === complianceFilter
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          ...
          const joinedDate = new Date(member.start_date ?? member.created_at).toLocaleDateString()
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {complianceStatus === "active" ? <ShieldCheck className="h-3 w-3" /> : <AlertCircle className="h-3 w-3" />}
                      {complianceStatus}
                    </div>
                  </div>
