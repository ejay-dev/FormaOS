...
export async function GET(request: Request) {
  try {
    ...
    const hours = Math.min(
      parseInt(url.searchParams.get('hours') || '24'),
      168,
    ); // max 7 days
    ...
    const failures: AutomationFailure[] = (auditLogs || []).map(
      (log: {
        id: string;
        ...
        errorCode:
          ((log.metadata as Record<string, unknown>)?.error_code as string) ||
          'UNKNOWN',
        errorMessage:
          ((log.metadata as Record<string, unknown>)
            ?.error_message as string) || 'Unknown error',
        timestamp: log.created_at,
        duration: (log.metadata as Record<string, unknown>)?.duration_ms as
          | number
          | undefined,
    ...
    const summary = {
      ...
      uniqueOrgs: new Set(failures.filter((f) => f.orgId).map((f) => f.orgId))
        .size,
      timeRange: {
      ...
      jobStats: jobMetrics.map((j) => ({
        ...
        failures: j.failures,
        failureRate:
          j.runs > 0 ? ((j.failures / j.runs) * 100).toFixed(1) + '%' : '0%',
