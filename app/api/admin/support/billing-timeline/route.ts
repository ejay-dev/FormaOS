...
export async function GET(request: Request) {
  try {
    ...
    const events: BillingEvent[] = (billingEvents || []).map(
    ...
    const subscriptionStates: SubscriptionState[] = (subscriptions || []).map(
      (sub: {
    ...
    const statusCounts = subscriptionStates.reduce(
      (acc, sub) => {
    ...
    const planCounts = subscriptionStates.reduce(
      (acc, sub) => {
