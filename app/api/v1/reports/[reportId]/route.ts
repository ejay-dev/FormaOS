...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (!report) {
    const response = jsonWithContext(auth.context, { error: 'Report not found' }, { status: 404 });
    await logV1Access(auth.context, 404, 'reports:read');
    return response;
  }

  const { searchParams } = new URL(request.url);
  if (searchParams.get('download') === '1' || searchParams.get('download') === 'true') {
    await logV1Access(auth.context, 302, 'reports:read');
    return NextResponse.redirect(report.file_url ?? `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/api/reports/export?type=${report.report_type}`);
  }
