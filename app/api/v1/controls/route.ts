...
const log = routeLog('/api/v1/controls');
...
export async function GET(request: Request) {
  try {
    ...
    if (q) {
      ...
      controlsQuery = controlsQuery.or(
        `title.ilike.${pattern},code.ilike.${pattern}`,
      );
    }
    ...
    const [
      ...
      auth.context.db
        ...
        .select(
          'control_key, status, compliance_score, evaluated_at, last_evaluated_at',
        )
        .eq('organization_id', auth.context.orgId),
    ]);
    ...
    const data = ((controls ?? []) as Array<Record<string, unknown>>).map(
      (control) => ({
        ...control,
        evidence_status: evidenceByControl.get(control.id as string) ?? {
          approved: 0,
          pending: 0,
          rejected: 0,
        },
