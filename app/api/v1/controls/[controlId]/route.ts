...
export async function GET(request: Request, context: RouteContext) {
  ...
  const [{ data: control }, { data: linkedEvidence }, { data: evaluations }] = await Promise.all([
    auth.context.db
  ...
  if (!control) {
    const response = jsonWithContext(auth.context, { error: 'Control not found' }, { status: 404 });
