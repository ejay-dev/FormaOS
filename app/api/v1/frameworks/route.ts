...
export async function GET(request: Request) {
  ...
  const [{ data: orgFrameworks }, { data: frameworks }, { data: evaluations }] = await Promise.all([
    auth.context.db
      .from('org_frameworks')
      .select('*', { count: 'exact' })
    ...
    auth.context.db
      ...
      .select('framework_id, compliance_score, total_controls, satisfied_controls, missing_controls, evaluated_at, last_evaluated_at')
      .eq('organization_id', auth.context.orgId),
  ]);
  ...
  const data = ((orgFrameworks ?? []) as Array<Record<string, unknown>>).map((installed) => {
    const framework = ((frameworks ?? []) as Array<Record<string, unknown>>).find(
      (item) =>
        item.slug === installed.framework_slug || item.id === installed.framework_slug,
    );
    ...
    return {
      ...
      coverage: evaluation
        ? {
            ...
            evaluated_at: evaluation.evaluated_at ?? evaluation.last_evaluated_at,
          }
        : null,
    };
  });
