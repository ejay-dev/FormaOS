...
export async function GET() {
  ...
  const snapshotScores = (snapshots ?? []).map((row: Record<string, unknown>) =>
  ...
  (controlEvals ?? []).forEach((row: Record<string, unknown>) => {
    const risk = (
      (row.details as Record<string, unknown>)?.risk_level ?? 'medium'
    )
      .toString()
      .toLowerCase();
  ...
  const combinedScore = readiness.length
    ? Math.round(
        readiness.reduce(
          (sum, fw) => sum + fw.readinessScore * Math.max(1, fw.totalControls),
          0,
        ) /
