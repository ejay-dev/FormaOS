/**
 * PCI-5 — Malware protection.
 *
 * Signal: compliance_scans with a malware/EDR-flavoured scan_type
 * within the last 90 days (pack cadence). Mirrors CIS-10.
 */
...
const evaluate: ControlEvaluator = async ({ orgId, db }) => {
      ...
      gaps: [
        {
          ...
          message:
            'No malware/EDR scans recorded — PCI-5 requires anti-malware on systems handling cardholder data.',
          severity: 'high',
        },
      ],
      confidence: 0.7,
      reason: `No malware scan_type in ${rows.length} compliance_scans row(s).`,
      evaluatedAt,
    };
  }
  ...
  if (sinceLast != null && sinceLast > RECENT_DAYS) {
    gaps.push({
      ...
      message: `Most recent malware/EDR scan was ${sinceLast}d ago — exceeds the ${RECENT_DAYS}d cadence.`,
  ...
  return {
    ...
    reason: `${malwareScans.length} malware scan(s); last completed ${sinceLast ?? '?'}d ago.`,
    evaluatedAt,
  };
};
