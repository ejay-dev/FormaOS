 ...
 *
 * Signal: MFA coverage across active org_members. PCI v4 requires
 * MFA for all access to the CDE; we approximate by requiring ≥95%
 * MFA coverage across active members.
 */
  ...
  if (coverage < 0.95) {
    gaps.push({
      ...
      message: `MFA enabled on ${withMfa.length}/${total} active members (${Math.round(
        coverage * 100,
      )}%) — PCI v4 requires MFA for all access to the CDE.`,
