/**
 * Audit server-actions-001 (2026-05-22): lib-level core for org
 * compliance evaluation. Does NOT verify caller authz — the server-
 * action wrapper at `@/app/app/actions/control-evaluations` enforces
 * the session→orgId match before delegating here. Calling this
 * directly is only safe from server-only code paths that already have
 * a trusted orgId (background workers / system flows).
 */
...
const REQUIRED_POLICIES: Array<{
...
}> = [
  {
    ...
    match: ['information security', 'security policy'],
    label: 'Information security policy',
  },
  {
    key: 'incident_management_policy',
    match: ['incident management', 'incident response', 'incident'],
    label: 'Incident management policy',
  },
  {
    key: 'access_control_policy',
    match: ['access control', 'identity access'],
    label: 'Access control policy',
  },
  {
    key: 'code_of_conduct',
    match: ['code of conduct', 'staff conduct', 'worker conduct'],
    label: 'Code of conduct',
  },
  {
    key: 'complaints_management',
    match: [
  ...
  },
  {
    ...
    match: ['patient privacy', 'privacy policy', 'confidentiality'],
    label: 'Patient privacy policy',
  },
  {
    key: 'infection_control_policy',
    match: ['infection control', 'hygiene', 'sterilization'],
    label: 'Infection control policy',
  },
  {
    key: 'data_breach_response_plan',
    match: ['data breach', 'breach response', 'incident response plan'],
    label: 'Data breach response plan',
  },
  {
    key: 'child_protection_policy',
    match: ['child protection', 'child safety', 'mandatory reporting'],
    label: 'Child protection policy',
  },
  {
    key: 'delivery_and_collection_of_children',
    match: [
      'delivery and collection',
      'collection of children',
      'arrival and departure',
    ],
    label: 'Delivery and collection of children',
  },
  {
    key: 'sun_protection_policy',
    match: ['sun protection', 'sun safety'],
    label: 'Sun protection policy',
  },
  {
    key: 'dignity_and_choice_policy',
    match: ['dignity and choice', 'consumer rights', 'resident choice'],
    label: 'Dignity and choice policy',
  },
  {
    key: 'clinical_governance_framework',
    match: ['clinical governance', 'governance framework'],
    label: 'Clinical governance framework',
  },
  {
    key: 'serious_incident_response_scheme',
    match: ['serious incident response scheme', 'sirs', 'incident reporting'],
    label: 'Serious incident response scheme',
  },
];
...
async function safeSelectTasks(
  ...
  for (const select of attempts) {
    try {
      ...
      if (!error) return (data ?? []) as unknown as RawTask[];
    } catch {
      // try next
    }
  }
...
async function safeSelectEvidence(
  supabase: SupabaseClient,
  orgId: string,
): Promise<RawEvidence[]> {
  const attempts = [
...
async function safeSelectPolicies(
  supabase: SupabaseClient,
  orgId: string,
): Promise<RawPolicy[]> {
  const attempts = [
  ...
  for (const select of attempts) {
    try {
      const { data, error } = await supabase
        ...
        .select(select)
        .eq('organization_id', orgId);
      if (!error) return (data ?? []) as unknown as RawPolicy[];
    } catch {
      // try next
    }
  }
...
export async function evaluateOrgComplianceCore(orgId: string) {
  try {
    if (!orgId) return;
