...
export async function processExportJob(
  ...
  try {
    ...
    const attempt = preclaimed
      ? (job.attempt_count ?? 0)
      : (job.attempt_count ?? 0) + 1;
    ...
    const manifest: ExportManifest = {
      ...
      complianceScore:
        scoreHistory[scoreHistory.length - 1]?.compliance_score || 0,
      contents: {
    ...
    const bucket =
      (process.env.COMPLIANCE_EXPORTS_BUCKET ?? '').trim() ||
      'compliance-exports';
    ...
    if (signed.error || !signed.data?.signedUrl) {
      throw new Error(
        `Signed URL failed: ${signed.error?.message ?? 'Unknown error'}`,
      );
    }
...
async function getEvidenceData(orgId: string, _frameworkSlug: string) {
  ...
  const { data: evidence } = await admin
    .from('org_evidence')
    .select(
      'id, title, evidence_type, verification_status, uploaded_at, file_url, metadata',
    )
    .eq('organization_id', orgId);
...
async function getPoliciesData(orgId: string) {
  ...
  const { data: policies } = await admin
    .from('org_policies')
    .select(
      'id, title, version, status, last_reviewed_at, approved_at, created_at',
    )
    .eq('organization_id', orgId);
...
async function createZipBundle(data: {
  manifest: unknown;
  controls: unknown;
  evidence: unknown;
  tasks: unknown;
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.append(JSON.stringify(data.controls, null, 2), {
      name: 'controls.json',
    });
    ...
    archive.append(JSON.stringify(data.policies, null, 2), {
      name: 'policies.json',
    });
    archive.append(JSON.stringify(data.automationLogs, null, 2), {
      name: 'automation-logs.json',
    });
    archive.append(JSON.stringify(data.scoreHistory, null, 2), {
