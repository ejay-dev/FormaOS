...
export async function buildMasterControlMappings(): Promise<{
  ...
  try {
    ...
    const { data: controls } = await admin.from('framework_controls').select(`
        id,
        control_code,
        title,
        summary_description,
    ...
    for (const [_title, groupControls] of titleGroups) {
      if (groupControls.length === 1) {
        ...
        const { data: master } = await admin.rpc(
          'find_or_create_master_control',
          {
...
export async function getFrameworksSatisfiedByControl(
  ...
  const enabledSlugs = new Set(
    (enabledFrameworks || []).map(
      (f: { framework_slug: string }) => f.framework_slug,
    ),
  );
