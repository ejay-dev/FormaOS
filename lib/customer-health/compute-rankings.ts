...
type OrgIdRow = { organization_id: string };
type LoginRow = { organization_id: string; created_at: string };
type ActivityRow = { organization_id: string; action: string | null };
type ComplianceRow = {
...
function emptyRankings(): HealthRankings {
...
/**
 * Compute health rankings for all active organizations.
 * Uses the admin (service-role) client so it must only be called from
 * founder-gated surfaces.
 */
export async function computeHealthRankings(
  adminClient: SupabaseClient,
): Promise<HealthRankings> {
  const { data: organizations, error: orgsError } = await adminClient
    .from('organizations')
    .select(
      `id, name, industry, created_at,
       org_subscriptions ( plan_key, status, trial_expires_at )`,
    )
  ...
  if (!orgRows.length) return emptyRankings();
  ...
  ((loginRows ?? []) as LoginRow[]).forEach((row) => {
    ...
    loginStatsByOrg.set(row.organization_id, {
  ...
  ((complianceRows ?? []) as ComplianceRow[]).forEach((row) => {
    ...
    if (typeof row.compliance_score === 'number' && list.length < 2) {
