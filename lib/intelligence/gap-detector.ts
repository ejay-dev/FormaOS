...
export async function detectComplianceGaps(
  ...
  const missingEvidence = (evaluations ?? [])
    .map((row: { control_key: string; details?: Record<string, unknown> }) => {
      const controlKey = row.control_key as string;
      const controlId = controlKey?.startsWith('control:')
        ? controlKey.split(':')[1]
        : null;
      if (controlId) controlIds.push(controlId);
      const required = Number(
        row.details?.required_evidence_count ??
          row.details?.requiredEvidenceCount ??
          0,
      );
      const approved = Number(
        row.details?.approved_evidence_count ??
          row.details?.approvedEvidenceCount ??
          0,
      );
  ...
  const taskControlIds = new Set(
    (taskLinks ?? []).map((row: { control_id: string }) => row.control_id),
  );
  ...
  if (soc2Framework?.id) {
    ...
    const soc2ControlIds =
      controls?.map((control: { id: string }) => control.id) ?? [];
