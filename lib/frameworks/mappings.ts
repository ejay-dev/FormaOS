...
export async function getControlMappingSummary(
  ...
  const controlIds = controls.map((control: { id: string }) => control.id);
  const { data: mappings } = await admin
    .from('control_mappings')
    .select(
      'internal_control_id, framework_slug, external_control_reference, mapping_strength',
    )
    .in('internal_control_id', controlIds);

  const mappingsByControl = new Map<string, ControlMappingSummary>();
  controls.forEach(
    (control: { id: string; control_code: string; title: string }) => {
  ...
  return Array.from(mappingsByControl.values()).map((summary) => {
    ...
    const coveredCount = mappedFrameworks.filter((slug) =>
      targetFrameworks.includes(slug),
    ).length;
