...
export async function bootstrapOrganizationAtomic(params: {
  ...
  try {
    ...
    if (orgError || !org?.id) {
      throw new Error(`Organization creation failed: ${orgError?.message || 'Unknown error'}`);
    }
    ...
    if (legacyError) {
      console.warn('[bootstrap] Legacy orgs entry failed (non-critical):', legacyError.message);
    }
    ...
    if (onboardingError) {
      console.warn('[bootstrap] Onboarding status failed (non-critical):', onboardingError.message);
    }
    ...
    if (subError) {
      console.warn('[bootstrap] Subscription creation failed (non-critical):', subError.message);
    }
    ...
    if (organizationId) {
      ...
      await admin.from('org_onboarding_status').delete().eq('organization_id', organizationId);
      await admin.from('org_subscriptions').delete().eq('organization_id', organizationId);
      await admin.from('org_members').delete().eq('organization_id', organizationId);
