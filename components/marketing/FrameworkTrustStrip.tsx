...
/* ════════════════════════════════════════════════════════════
   Framework data — monochrome. No per-category colour identity;
   icons are uniform slate so the framework names carry the strip.
   ════════════════════════════════════════════════════════════ */
...
const ROW_1: FrameworkItem[] = [
  { icon: <Heart size={14} />, label: 'NDIS Practice Standards' },
  { icon: <Heart size={14} />, label: 'Aged Care Quality Standards' },
  { icon: <ShieldCheck size={14} />, label: 'NSQHS Standards' },
  { icon: <Shield size={14} />, label: 'AHPRA' },
  { icon: <CreditCard size={14} />, label: 'ASIC s912A' },
  { icon: <Shield size={14} />, label: 'APRA CPS 230' },
  { icon: <FileCheck size={14} />, label: 'AUSTRAC AML/CTF' },
  { icon: <Layers size={14} />, label: 'ACECQA NQF' },
  { icon: <ShieldCheck size={14} />, label: 'WHS Act' },
];

/* Row 2 — International standards, scrolls right */
const ROW_2: FrameworkItem[] = [
  { icon: <Shield size={14} />, label: 'SafeWork Australia' },
  { icon: <Shield size={14} />, label: 'ISO 27001' },
  { icon: <ShieldCheck size={14} />, label: 'SOC 2' },
  { icon: <Globe size={14} />, label: 'GDPR' },
  { icon: <Layers size={14} />, label: 'NIST CSF' },
  { icon: <CreditCard size={14} />, label: 'PCI DSS' },
  { icon: <Heart size={14} />, label: 'HIPAA' },
  { icon: <FileCheck size={14} />, label: 'CIS Controls' },
  { icon: <Shield size={14} />, label: 'ISO 9001' },
];

/* ════════════════════════════════════════════════════════════
   Badge component — monochrome surface, hairline border, quiet hover
   ════════════════════════════════════════════════════════════ */
...
const FrameworkBadge = memo(function FrameworkBadge({
  ...
  return (
    <span className="group inline-flex items-center gap-2.5 rounded-full border border-white/[0.08] bg-white/[0.025] px-4 py-2.5 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06] hover:-translate-y-px">
      <span className="flex-shrink-0 inline-flex items-center justify-center w-6 h-6 rounded-full bg-white/[0.05] text-slate-300 transition-colors duration-300">
        {icon}
      </span>
