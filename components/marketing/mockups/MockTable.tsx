    ...
    <div className="rounded-lg border border-white/[0.08] bg-[#0d1428] overflow-hidden">
      ...
      >
        {columns.map((col) => (
          <div
            key={col.key}
            className="px-2 py-1.5 text-[6px] uppercase tracking-wider font-bold text-white/20"
          >
            {col.label}
          </div>
        ))}
      ...
      {rows.map((row, idx) => (
        <motion.div
          ...
          className="grid border-b border-white/[0.04] last:border-0 transition-colors"
          style={{
            gridTemplateColumns: `${columns.map((_, i) => (i === 0 ? '2fr' : '1fr')).join(' ')}${showEvidence ? ' 0.5fr' : ''}`,
            height: 32,
          }}
            ...
            >
              {col.key === 'status' ? (
                <span className="flex items-center gap-1">
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${statusDot[row.status]} flex-shrink-0`}
                  />
                  <span className={`text-[8px] ${statusText[row.status]}`}>
                    {row.cells[col.key]}
                  </span>
                </span>
              ) : (
