    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-3">
      ...
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground/60">{demoPolicies.length} total</span>
        </div>
      ...
      <motion.div variants={fadeUp} className="rounded-xl bg-white/[0.02] border border-white/[0.06] overflow-hidden">
        {/* Header row */}
        <div className="grid grid-cols-[1fr_90px_60px_70px_70px] gap-2 px-3 py-2 bg-white/[0.02] border-b border-white/[0.06] text-[9px] font-semibold text-muted-foreground/60 uppercase tracking-wider">
        ...
        {demoPolicies.map((policy, i) => (
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-foreground/90 truncate font-medium">{policy.name}</span>
            </div>
            <span className="text-muted-foreground truncate">{policy.owner.split(' ')[0]}</span>
            <StatusBadge status={policy.status} />
            <span className="text-muted-foreground/60 hidden sm:block text-[9px]">{policy.lastReview}</span>
            <span className="text-muted-foreground/60 hidden sm:block text-[9px]">{policy.nextReview}</span>
          </motion.div>
        ))}
      </motion.div>
