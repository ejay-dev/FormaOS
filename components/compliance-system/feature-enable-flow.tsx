...
const MODULE_CONFIG: Record<ModuleId, {
  name: string;
  description: string;
...
function ModuleNode({ moduleId, state, onAction, isProcessing }: ModuleNodeProps) {
...
export function FeatureEnableFlow({ onModuleSelect }: FeatureEnableFlowProps) {
  const { state, getModuleState, isModuleAccessible, getPlan } = useSystemState();
  const { reportSuccess, reportWarning, reportInfo } = useComplianceAction();
  const [processingModule, setProcessingModule] = useState<ModuleId | null>(null);
  const [recentlyActivated, setRecentlyActivated] = useState<ModuleId | null>(null);

  const plan = getPlan();
  const enabledModules = PLAN_FEATURES[plan];

  // Primary modules (always shown)
  const primaryModules: ModuleId[] = ["controls", "evidence", "policies", "vault"];
  
  // Advanced modules
  const advancedModules: ModuleId[] = ["audits", "registers", "team", "reports", "tasks"];
  
  // Admin modules
  const adminModules: ModuleId[] = ["admin", "billing", "settings"];

  const handleModuleAction = useCallback(async (moduleId: ModuleId) => {
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className={cn(
          ...
          plan === "enterprise" && "bg-gradient-to-r from-violet-500/20 to-purple-500/20 text-violet-300 border border-violet-400/30",
          plan === "pro" && "bg-gradient-to-r from-cyan-500/20 to-teal-500/20 text-cyan-300 border border-cyan-400/30",
          plan === "basic" && "bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-300 border border-blue-400/30",
          plan === "trial" && "bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-300 border border-amber-400/30"
        )}>
          {plan}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {primaryModules.map(moduleId => (
            <ModuleNode
              key={moduleId}
              moduleId={moduleId}
              state={recentlyActivated === moduleId ? "processing" : getModuleState(moduleId)}
