  ...
  return (
    <div className="bg-amber-400/10 border border-amber-400/30 rounded-[2.5rem] p-8 space-y-6 shadow-sm animate-in slide-in-from-right-4 duration-500">
      ...
      <div className="space-y-3">
        {atRiskDocs.map((doc) => (
          <div key={doc.id} className="bg-glass-strong backdrop-blur-md rounded-2xl p-4 flex items-center justify-between border border-amber-400/30/50 group hover:bg-glass-strong transition-all">
            <div className="flex items-center gap-4">
                <div className="h-10 w-10 rounded-xl bg-amber-400/10 flex items-center justify-center text-amber-300">
                    <User className="h-5 w-5" />
                </div>
                <div>
                    <p className="text-xs font-black text-foreground">{doc.document_type}</p>
            ...
            <Link href={`/app/people`} className="p-2 text-muted-foreground hover:text-foreground transition-colors">
