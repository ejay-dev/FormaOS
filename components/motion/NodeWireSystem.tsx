...
const MODULES: ModuleNode[] = [
  ...
  },
  {
    ...
    requiredPlan: 'trial',
    description: 'Store artifacts securely',
  },
  {
    id: 'tasks',
    name: 'Tasks',
  ...
  },
  {
    ...
    requiredPlan: 'basic',
    description: 'Version-controlled library',
  },
  {
    id: 'analytics',
    name: 'Analytics',
  ...
  },
  {
    ...
    description: 'Full governance suite',
  },
];
...
function AnimatedNode({
  ...
  const statusStyles = {
    ...
    upgrade:
      'border border-current border-dashed opacity-60 cursor-pointer hover:opacity-100',
  };

  const statusIcons = {
    locked: <Lock className="h-3 w-3 absolute -top-1 -right-1 text-white/50" />,
    upgrade: (
      <ArrowRight className="h-3 w-3 absolute -top-1 -right-1 text-amber-400" />
    ),
    processing: (
      <Loader2 className="h-3 w-3 absolute -top-1 -right-1 animate-spin" />
    ),
    active: (
      <Check className="h-3 w-3 absolute -top-1 -right-1 text-emerald-400" />
    ),
    restricted: (
      <AlertTriangle className="h-3 w-3 absolute -top-1 -right-1 text-rose-400" />
    ),
    preview: null,
  };
    ...
    >
      ...
      <div
        ...
        style={{
          color: module.color,
          backgroundColor:
            status === 'active' ? `${module.color}20` : 'transparent',
        }}
      >
      ...
      <motion.span
        ...
        style={{
          color: status === 'active' ? module.color : 'rgba(255,255,255,0.6)',
        }}
      >
        {module.name}
      </motion.span>
...
function AnimatedWire({
  ...
  const _length = Math.sqrt(dx * dx + dy * dy);
    ...
    >
      <defs>
        ...
        >
          <stop
            offset="0%"
            stopColor={active ? 'rgb(56, 189, 248)' : 'rgba(255,255,255,0.1)'}
          />
          <stop
            offset="50%"
            stopColor={active ? 'rgb(139, 92, 246)' : 'rgba(255,255,255,0.2)'}
          />
          <stop
            offset="100%"
            stopColor={active ? 'rgb(6, 182, 212)' : 'rgba(255,255,255,0.1)'}
          />
        </linearGradient>
      </defs>
  ...
  const wires = useMemo((): WireConnection[] => {
    const connections: WireConnection[] = [
      ...
      },
      {
        from: 'policies',
        to: 'analytics',
        active: plan === 'pro' || plan === 'enterprise',
        animating: false,
      },
      {
        from: 'analytics',
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      <div className="absolute inset-0">
        {Object.keys(modulePositions).length > 0 &&
          wires.map((wire, _idx) => (
            <AnimatedWire
      ...
      {/* Nodes grid */}
      <div
        className={`relative z-10 grid ${compact ? 'grid-cols-4 gap-4' : 'grid-cols-4 gap-8'}`}
      >
