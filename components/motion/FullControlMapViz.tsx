...
const FRAMEWORK_COLORS: Record<string, { fill: string; glow: string; ring: string; label: string }> = {
  'iso27001': { fill: 'rgba(20,184,166,0.92)', glow: 'rgba(20,184,166,0.30)', ring: 'rgba(20,184,166,0.45)', label: 'rgba(94,234,212,1)' },
  'soc2':     { fill: 'rgba(99,102,241,0.90)', glow: 'rgba(99,102,241,0.28)', ring: 'rgba(129,140,248,0.45)', label: 'rgba(165,180,252,1)' },
  'nist':     { fill: 'rgba(56,189,248,0.90)', glow: 'rgba(56,189,248,0.28)', ring: 'rgba(125,211,252,0.45)', label: 'rgba(186,230,253,1)' },
  'hipaa':    { fill: 'rgba(244,114,182,0.88)', glow: 'rgba(244,114,182,0.25)', ring: 'rgba(251,146,60,0.45)', label: 'rgba(253,186,116,1)' },
  'gdpr':     { fill: 'rgba(167,139,250,0.90)', glow: 'rgba(167,139,250,0.28)', ring: 'rgba(196,181,253,0.45)', label: 'rgba(221,214,254,1)' },
  'pci':      { fill: 'rgba(251,146,60,0.88)', glow: 'rgba(251,146,60,0.25)', ring: 'rgba(253,186,116,0.45)', label: 'rgba(254,215,170,1)' },
  'cis':      { fill: 'rgba(52,211,153,0.90)', glow: 'rgba(52,211,153,0.28)', ring: 'rgba(110,231,183,0.45)', label: 'rgba(167,243,208,1)' },
};

const TYPE_ALPHA: Record<NodeType, number> = { framework: 1, control: 0.9, evidence: 0.8, task: 0.75 };
const TYPE_ICONS: Record<NodeType, string> = { framework: 'F', control: 'C', evidence: 'E', task: 'T' };
...
const FRAMEWORKS: FrameworkDef[] = [
  {
    id: 'iso27001', label: 'ISO 27001', angle: 0,
    controls: [
  ...
  {
    id: 'soc2', label: 'SOC 2', angle: 51.4,
    controls: [
      ...
      { id: 'soc-class', label: 'Data Classification', category: 'Confidentiality' },
  ...
  {
    id: 'nist', label: 'NIST CSF', angle: 102.8,
    controls: [
  ...
  {
    id: 'hipaa', label: 'HIPAA', angle: 154.3,
    controls: [
  ...
  {
    id: 'gdpr', label: 'GDPR', angle: 205.7,
    controls: [
  ...
  {
    id: 'pci', label: 'PCI DSS', angle: 257.1,
    controls: [
  ...
  {
    id: 'cis', label: 'CIS Controls', angle: 308.6,
    controls: [
...
const TOTAL_CONTROLS = FRAMEWORKS.reduce((sum, fw) => sum + fw.controls.length, 0);

/* ── Cross-framework shared control edges ─────────────────── */

const CROSS_EDGES: MapEdge[] = [
  // Access control linkages
  { from: 'iso-ac', to: 'soc-iam' }, { from: 'iso-ac', to: 'nist-ac' }, { from: 'iso-ac', to: 'hip-ac' },
  { from: 'iso-ac', to: 'pci-ac' }, { from: 'iso-ac', to: 'cis-acl' },
  // Encryption linkages
  { from: 'iso-crypto', to: 'soc-enc' }, { from: 'iso-crypto', to: 'nist-dp' },
  { from: 'iso-crypto', to: 'hip-tx' }, { from: 'iso-crypto', to: 'pci-tx' },
  // Incident response linkages
  { from: 'iso-inc', to: 'soc-incr' }, { from: 'iso-inc', to: 'nist-irp' },
  { from: 'iso-inc', to: 'hip-inc' }, { from: 'iso-inc', to: 'cis-ir' },
  // Risk assessment linkages
  { from: 'iso-risk', to: 'nist-ra' }, { from: 'iso-risk', to: 'hip-risk' },
  { from: 'iso-risk', to: 'soc-gov' },
  // Monitoring
  { from: 'iso-log', to: 'soc-mon' }, { from: 'iso-log', to: 'nist-cm' },
  { from: 'iso-log', to: 'pci-log' }, { from: 'iso-log', to: 'cis-audit' },
  // Training
  { from: 'iso-aware', to: 'nist-train' }, { from: 'iso-aware', to: 'hip-train' },
  { from: 'iso-aware', to: 'soc-train' }, { from: 'iso-aware', to: 'cis-train' },
  // Vendor
  { from: 'iso-sup', to: 'gdpr-vendor' }, { from: 'iso-sup', to: 'soc-vendor' },
  { from: 'iso-sup', to: 'nist-supply' }, { from: 'iso-sup', to: 'cis-vendor' },
  // Business continuity
  { from: 'iso-bc', to: 'soc-bcp' }, { from: 'iso-bc', to: 'nist-rec' },
  { from: 'iso-bc', to: 'hip-cont' },
  // Data protection cross-links
  { from: 'gdpr-min', to: 'soc-class' }, { from: 'gdpr-min', to: 'nist-dp' },
  { from: 'gdpr-breach', to: 'hip-breach' },
  { from: 'pci-malware', to: 'cis-mal' }, { from: 'pci-dev', to: 'cis-appsec' },
  { from: 'pci-config', to: 'cis-config' }, { from: 'pci-net', to: 'cis-netmgmt' },
  { from: 'nist-anom', to: 'cis-netmon' }, { from: 'nist-anom', to: 'soc-mon' },
  { from: 'soc-dr', to: 'hip-cont' }, { from: 'soc-dr', to: 'nist-rec' },
  // Change management
  { from: 'iso-change', to: 'soc-chg' },
  // Privacy
  { from: 'gdpr-dsr', to: 'soc-rights' }, { from: 'gdpr-consent', to: 'soc-priv' },
];

/* ── Layout: radial galaxy layout ─────────────────────────── */

function buildGalaxy(w: number, h: number): { nodes: MapNode[]; edges: MapEdge[] } {
  const cx = w / 2;
  const cy = h / 2;
  const maxR = Math.min(w, h) * 0.44;
  const fwRadius = maxR * 0.18;
  const innerRing = maxR * 0.48;
  const outerRing = maxR * 0.78;
  const edgeRing = maxR * 0.96;
  ...
  for (const fw of FRAMEWORKS) {
    ...
    const fwX = cx + Math.cos(fwAngle) * fwRadius;
    const fwY = cy + Math.sin(fwAngle) * fwRadius;
    ...
    const arcSpan = (2 * Math.PI) / FRAMEWORKS.length * 0.85;
    ...
    fw.controls.forEach((ctrl, i) => {
      ...
      // Alternate between inner and outer rings for density
      const ringIdx = i % 3;
      const radius = ringIdx === 0 ? innerRing : ringIdx === 1 ? outerRing : (innerRing + outerRing) / 2;
      // Add slight radial jitter
      const jitter = (Math.sin(i * 7.3 + fw.angle) * 0.08 + 1) * radius;
      const ctrlX = cx + Math.cos(angle) * jitter;
      const ctrlY = cy + Math.sin(angle) * jitter;
      ...
      nodes.push({
        ...
        x: ctrlX, y: ctrlY,
        baseX: ctrlX, baseY: ctrlY,
        size: Math.max(6, maxR * 0.016),
        phase: angle + i * 0.3,
...
function quadCP(x1: number, y1: number, x2: number, y2: number, cx: number, cy: number, pull: number) {
  const mx = (x1 + x2) / 2;
  const my = (y1 + y2) / 2;
  return { cpx: mx + (cx - mx) * pull, cpy: my + (cy - my) * pull };
}

function quadPt(t: number, x1: number, y1: number, cpx: number, cpy: number, x2: number, y2: number) {
  const u = 1 - t;
...
function FullControlMapVizInner({ className = '' }: FullControlMapVizProps) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const observer = new IntersectionObserver(([entry]) => setIsInView(entry.isIntersecting), { rootMargin: '300px 0px' });
    observer.observe(el);
  ...
  useEffect(() => {
    ...
    if (!dataRef.current || Math.abs(sizeRef.current.width - w) > 15 || Math.abs(sizeRef.current.height - h) > 15) {
    ...
    const animate = (timestamp: number) => {
      ...
      const halo = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, unit * 0.55);
      ...
      for (const p of particles) {
        if (!shouldReduceMotion) { p.x += p.vx; p.y += p.vy; if (p.x < 0) p.x = w; if (p.x > w) p.x = 0; if (p.y < 0) p.y = h; if (p.y > h) p.y = 0; }
        const pa = p.alpha * (0.5 + 0.5 * Math.sin(tSec * 0.4 + p.phase));
        ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(148,163,184,${pa})`; ctx.fill();
      }

      /* ── Layer 3: Concentric orbit rings ── */
      const orbitCounts = 6;
      ctx.save();
      for (let i = 0; i < orbitCounts; i++) {
        const r = unit * (0.08 + i * 0.075);
        ctx.beginPath(); ctx.ellipse(centerX, centerY, r * 1.15, r, 0, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(148,163,184,${0.04 - i * 0.004})`;
        ctx.lineWidth = 0.6;
        ctx.setLineDash([3, 14 + i * 3]);
        ctx.lineDashOffset = shouldReduceMotion ? 0 : tSec * (5 + i * 1.5) * (i % 2 === 0 ? 1 : -1);
        ctx.stroke();
      }
      ...
      if (!shouldReduceMotion) {
        const ang = tSec * 0.2;
        const sLen = unit * 0.5;
        const sx = centerX + Math.cos(ang) * sLen;
        const sy = centerY + Math.sin(ang) * sLen;
        ...
        ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(sx, sy);
        ctx.strokeStyle = sg; ctx.lineWidth = unit * 0.18; ctx.stroke();
      }
      ...
      if (hoveredNode) {
        ...
        edges.forEach((edge, idx) => {
          if (edge.from === hoveredNode.id || edge.to === hoveredNode.id) {
            connectedIds.add(edge.from); connectedIds.add(edge.to);
      ...
      edges.forEach((edge, idx) => {
        ...
        const cp = quadCP(fromN.x, fromN.y, toN.x, toN.y, centerX, centerY, pull);
        ctx.beginPath();
        ...
        if (isHL) {
          ...
          eg.addColorStop(0, fc.ring); eg.addColorStop(1, tc.ring);
          ctx.strokeStyle = eg; ctx.lineWidth = isCross ? 2 : 1.5;
        } else {
        ...
        if (!shouldReduceMotion && !isDim) {
          ...
          for (let p = 0; p < pCount; p++) {
            ...
            const pos = quadPt(phase, fromN.x, fromN.y, cp.cpx, cp.cpy, toN.x, toN.y);
            const sz = isHL ? 3 : 1.8;
            const al = isHL ? 0.7 : 0.2;

            // Glow
            const pg = ctx.createRadialGradient(pos.x, pos.y, 0, pos.x, pos.y, sz * 3);
            pg.addColorStop(0, colors.fill.replace(/[\d.]+\)$/, `${al * 0.5})`));
            pg.addColorStop(1, 'rgba(0,0,0,0)');
            ctx.fillStyle = pg; ctx.beginPath(); ctx.arc(pos.x, pos.y, sz * 3, 0, Math.PI * 2); ctx.fill();

            // Core
            ctx.beginPath(); ctx.arc(pos.x, pos.y, sz, 0, Math.PI * 2);
            ctx.fillStyle = colors.fill.replace(/[\d.]+\)$/, `${al})`); ctx.fill();

            // Trail
            if (isHL) {
              for (let tr = 1; tr <= 4; tr++) {
                const trT = Math.max(0, phase - tr * 0.018);
                const trP = quadPt(trT, fromN.x, fromN.y, cp.cpx, cp.cpy, toN.x, toN.y);
                ctx.beginPath(); ctx.arc(trP.x, trP.y, sz * (1 - tr * 0.18), 0, Math.PI * 2);
                ctx.fillStyle = colors.fill.replace(/[\d.]+\)$/, `${al * (1 - tr * 0.22)})`); ctx.fill();
              }
            }
          }
        }
      ...
      if (hoveredNode && !shouldReduceMotion) {
        ...
        ctx.beginPath(); ctx.arc(hoveredNode.x, hoveredNode.y, rr, 0, Math.PI * 2);
      ...
      for (const node of nodes) {
        ...
        if (!isDimmed) {
          ...
          const pulse = shouldReduceMotion ? 1 : 0.85 + 0.15 * Math.sin(tSec * 1.2 + node.phase);
          const hg = ctx.createRadialGradient(node.x, node.y, ns * 0.3, node.x, node.y, haloR * pulse);
          hg.addColorStop(0, colors.glow.replace(/[\d.]+\)$/, `${isHovered ? 0.35 : isFW ? 0.15 : 0.08})`));
          hg.addColorStop(1, 'rgba(0,0,0,0)');
          ctx.fillStyle = hg; ctx.beginPath(); ctx.arc(node.x, node.y, haloR * pulse, 0, Math.PI * 2); ctx.fill();
        }

        // Outer dashed ring (frameworks + hovered)
        if ((isFW || isHovered) && !isDimmed && !shouldReduceMotion) {
          ctx.save(); ctx.beginPath(); ctx.arc(node.x, node.y, ns + 4, 0, Math.PI * 2);
          ctx.strokeStyle = colors.ring.replace(/[\d.]+\)$/, `${isHovered ? 0.6 : 0.25})`);
          ctx.lineWidth = 0.8; ctx.setLineDash([2.5, 4]); ctx.lineDashOffset = tSec * 6 * (node.phase > 2 ? -1 : 1);
          ctx.stroke(); ctx.setLineDash([]); ctx.restore();
        }

        // Fill
        const fg = ctx.createRadialGradient(node.x - ns * 0.15, node.y - ns * 0.15, 0, node.x, node.y, ns);
        fg.addColorStop(0, colors.fill.replace(/[\d.]+\)$/, `${alpha * 0.95})`));
        fg.addColorStop(1, colors.fill.replace(/[\d.]+\)$/, `${alpha * 0.6})`));
        ctx.beginPath(); ctx.arc(node.x, node.y, ns, 0, Math.PI * 2); ctx.fillStyle = fg; ctx.fill();
      ...
      /* ── HUD: Corner brackets ── */
      const bs = 18; const bi = 6;
      ctx.strokeStyle = 'rgba(148,163,184,0.10)'; ctx.lineWidth = 1;
      [[bi, bi + bs, bi, bi, bi + bs, bi], [w - bi - bs, bi, w - bi, bi, w - bi, bi + bs],
       [bi, h - bi - bs, bi, h - bi, bi + bs, h - bi], [w - bi - bs, h - bi, w - bi, h - bi, w - bi, h - bi - bs]].forEach(([ax, ay, bx, by, ex, ey]) => {
        ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(bx, by); ctx.lineTo(ex, ey); ctx.stroke();
      });

      /* ── HUD: Framework angle ticks ── */
    ...
    return (
      <div ref={containerRef} className={`relative ${className}`}>
        <div className="flex items-center justify-center h-full text-gray-500 text-sm">
          Full compliance control map ({CONTROL_MAP_STATS.totalNodes} nodes — motion reduced)
        </div>
      </div>
    );
  }
