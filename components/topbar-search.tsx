...
export function TopbarSearch() {
  ...
  const [results, setResults] = useState<
    { id: string; title: string; type: 'policy' | 'task' | 'evidence' }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const searchData = async () => {
      ...
      try {
        ...
        const combined = (json?.results ?? []) as {
          id: string;
          title: string;
          type: 'policy' | 'task' | 'evidence';
        }[];
        setResults(combined);
        setLoading(false);
        setIsOpen(true);
          ...
          {results.map((result) => (
            ...
            >
              ...
              <div className="flex-1 truncate">
                <span className="font-medium text-card-foreground block truncate">
                  {result.title}
                </span>
