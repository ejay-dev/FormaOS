  ...
  return (
    <div className="relative w-full max-w-md hidden md:block" ref={wrapperRef}>
      <div className="relative group">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 group-focus-within:text-slate-100 transition-colors" />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onFocus={() => query.length > 1 && setIsOpen(true)}
          placeholder="Search policies, tasks..."
          className="w-full rounded-full border border-white/10 bg-white/10 py-2 pl-10 pr-10 text-sm text-slate-100 outline-none focus:border-white/20 focus:bg-white/10 focus:ring-4 focus:ring-sky-500/20 transition-all placeholder:text-slate-400"
        />
        {loading ? (
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute mt-2 w-full rounded-2xl border border-white/10 bg-[#0B1220] p-2 shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {results.map((result) => (
            <Link
              ...
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-white/10 transition-colors group"
            >
              {result.type === 'policy' ? (
                <div className="h-8 w-8 rounded-lg bg-white/10 text-sky-200 flex items-center justify-center">
                    <FileText className="h-4 w-4" />
                </div>
              ) : (
                <div className="h-8 w-8 rounded-lg bg-white/10 text-emerald-200 flex items-center justify-center">
                    <CheckSquare className="h-4 w-4" />
                </div>
              )}
              <div className="flex-1 truncate">
                 <span className="font-medium text-slate-100 block truncate">{result.title}</span>
                 <span className="text-[10px] text-slate-400 capitalize">{result.type}</span>
              </div>
            </Link>
          ))}
        </div>
