          ...
          return (
            <div key={milestone.id} className="relative flex gap-4">
              ...
              <div className={`pb-6 ${isLast ? 'pb-0' : ''}`}>
                <div className="text-sm font-semibold text-foreground/90">{milestone.title}</div>
                {milestone.description && (
                  <div className="mt-0.5 text-xs text-muted-foreground/60">{milestone.description}</div>
                )}
