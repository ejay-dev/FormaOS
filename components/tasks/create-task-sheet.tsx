...
export function CreateTaskSheet() {
  ...
  const onSubmit = async (
    ...
    formData.append('dueDate', String(data.dueDate ?? ''));
    if (data.isRecurring) {
      formData.append('recurrenceDays', String(data.recurrenceDays ?? ''));
    }
    ...
    try {
      ...
      nodeCreated('task', String(data.title ?? ''));
        ...
        {success ? (
          ...
          <>
            <SheetHeader className="mb-6">
              ...
              <SheetDescription>
                Assign a new action item to your compliance roadmap.
              </SheetDescription>
            </SheetHeader>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="p-4 bg-purple-500/10 rounded-xl border border-purple-400/30 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <label
                    htmlFor="rec"
                    className="text-sm font-bold text-purple-200 flex items-center gap-2 cursor-pointer select-none"
                  >
                ...
                {isRecurring && (
                  <div className="animate-in fade-in slide-in-from-top-2">
                    <label
                      htmlFor="field-95"
                      className="text-xs font-bold uppercase text-purple-300 tracking-wider block mb-2"
                    >
                      Repeat Every (Days)
                    </label>
                    <div className="flex gap-2">
                      ...
                      />
                      <div className="flex items-center text-xs font-medium text-purple-300">
                        Days
                      </div>
                    </div>
                    <p className="text-xs text-purple-300/70 mt-2">
                      Example: 30 = Monthly, 90 = Quarterly, 365 = Yearly
                    </p>
                  </div>
                )}
              </div>
