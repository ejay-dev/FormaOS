...
const ROUTES: { slug: string; path: string }[] = [
  ...
  { slug: 'contact-compliance-plan', path: '/contact?type=compliance-plan&plan=growth&source=pricing' },
  { slug: 'contact-enterprise', path: '/contact?type=enterprise&plan=enterprise&source=pricing' },
  ...
  { slug: 'financial-services-compliance', path: '/financial-services-compliance' },
...
test.beforeAll(() => {
  ...
  for (const f of fs.readdirSync(OUTPUT_DIR)) {
    try { if (f.endsWith('.png')) fs.unlinkSync(path.join(OUTPUT_DIR, f)); } catch { /* already gone */ }
  }
});
