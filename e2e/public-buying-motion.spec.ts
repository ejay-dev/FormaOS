...
test.describe('Public buying motion', () => {
  test('homepage uses compliance-plan and demo CTAs', async ({ page }) => {
    ...
    const secondary = page
      .getByRole('link', { name: /Book Demo|See Demo/i })
      .first();
  ...
  test('pricing leads with compliance plan, self-serve foundation, and enterprise demo paths', async ({
    page,
  }) => {
  ...
  test('contact page is sales/demo led', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Talk to Sales|Book Demo|Request a Demo/i).first(),
    ).toBeVisible();
  });
  ...
  test('compare page avoids trial funnel copy', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: /Start Buyer Review/i }).first(),
    ).toHaveAttribute('href', /\/contact\?type=procurement/);
  });

  test('industry page uses compliance plan and demo motion', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('link', { name: /See Demo|Book Demo/i }).first(),
    ).toBeVisible();
  });

  test('header and footer CTAs route to guided contact flows', async ({
    page,
  }) => {
    await expectInfrastructureBuyingMotion(page, '/');
    ...
    const footerCta = page
      .locator('footer')
      .getByRole('link', { name: /Get Compliance Plan/i });
    await expect(footerCta).toHaveAttribute(
      'href',
      /\/contact\?type=compliance-plan/,
    );
    const footerSales = page
      .locator('footer')
      .getByRole('link', { name: /Talk to Sales/i });
    await expect(footerSales).toHaveAttribute('href', /\/contact\?type=sales/);
  });

  test('mobile nav CTAs avoid signup and route to sales/compliance plan', async ({
    page,
  }) => {
    ...
    await expect(mobilePrimary).toHaveAttribute(
      'href',
      /\/contact\?type=compliance-plan/,
    );

    const mobileSales = page
      .getByRole('dialog', { name: /Navigation menu/i })
      .getByRole('link', {
  ...
  test('security and trust routes use review/packet CTAs', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: /Book Security Review/i }).first(),
    ).toHaveAttribute('href', /\/contact\?type=security-review/);

    await expectInfrastructureBuyingMotion(page, '/trust');
    await expect(
      page.getByRole('link', { name: /Download Trust Packet/i }).first(),
    ).toHaveAttribute('href', /\/trust\/packet/);
  });
});
