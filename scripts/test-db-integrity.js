  ...
  const tests = [
    {
      ...
      test: async () => {
        ...
        if (orphanError) {
          ...
          return {
            status: orphaned.length === 0 ? 'Consistent' : 'Issues Found',
        ...
        return {
          status: orphanedUsers?.length === 0 ? 'Consistent' : 'Issues Found',
    ...
    },
    {
      ...
      test: async () => {
        ...
        const { data: roleAssignments, error } = await supabase.from(
          'user_roles',
        ).select(`
            id,
            role,
            user_profile:user_profiles!user_id(id)
          `);
        ...
        const invalidRoles =
          roleAssignments?.filter(
            (assignment) =>
              ...
              !['admin', 'compliance_manager', 'employee'].includes(
                assignment.role,
              ),
          ) || [];
